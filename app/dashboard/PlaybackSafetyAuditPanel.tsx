  ...
  async function refreshAudit() {
    ...
    try {
      const response = await fetch(
        "/api/spotify/playback/audit?limit=12",
      );
      ...
      {rows.length === 0 ? (
        ...
        <div className="space-y-2">
          {rows.map((row) => (
            <div
              key={row.id}
              className="rounded-xl border border-white/10 bg-black/35 p-3 text-xs md:text-sm"
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="font-semibold text-white">
                  {row.command_type.replaceAll(
                    "_",
                    " ",
                  )}{" "}
                  -{" "}
              ...
              {row.command_payload
                ...
                <p className="mt-1 text-amber-200">
                  Guardrails:{" "}
                  {row.command_payload.guardrails
                    .map(
                      (g) => g.code,
                    )
                    .join(", ")}
