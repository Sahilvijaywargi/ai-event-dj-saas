...
export function SpotifyIntegrationPanel({
  ...
  async function syncPlaylists() {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.message ?? "Failed to sync playlists.");
      ...
      setSyncStatus("error");
      setErrorMessage(error instanceof Error ? error.message : "Failed to sync playlists.");
    }
  }
  ...
  return (
    <article id="spotify" className="glass-panel animate-fade-up rounded-2xl p-5 md:p-6">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold md:text-2xl">Spotify Integration</h2>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl border border-white/10 bg-black/35 p-3">
          <p className="text-xs uppercase tracking-widest text-white/60">Connection</p>
          <p className="mt-1 font-semibold">{connected ? "Connected" : "Not connected"}</p>
        </div>
        <div className="rounded-xl border border-white/10 bg-black/35 p-3">
          <p className="text-xs uppercase tracking-widest text-white/60">Account</p>
          <p className="mt-1 font-semibold">{accountName ?? "-"}</p>
        </div>
        <div className="rounded-xl border border-white/10 bg-black/35 p-3">
          <p className="text-xs uppercase tracking-widest text-white/60">Playlist Count</p>
          <p className="mt-1 font-semibold">{playlists.length}</p>
        </div>
        <div className="rounded-xl border border-white/10 bg-black/35 p-3">
          <p className="text-xs uppercase tracking-widest text-white/60">Sync Status</p>
      ...
      {connected ? (
        <>
          <form onSubmit={runSearch} className="mt-4 grid gap-2 sm:grid-cols-[1fr_auto_auto]">
            <input
              value={query}
            ...
            <select
              value={searchType}
              onChange={(event) =>
                setSearchType(event.target.value as "track" | "artist" | "playlist")
              }
              className="rounded-xl border border-white/20 bg-white/5 px-3 py-2 text-sm"
            >
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <div className="rounded-xl border border-white/10 bg-black/35 p-3">
              <p className="text-xs uppercase tracking-widest text-white/60">Playlists</p>
            ...
            <div className="rounded-xl border border-white/10 bg-black/35 p-3">
              <p className="text-xs uppercase tracking-widest text-white/60">Search Results</p>
              <ul className="mt-2 space-y-1 text-sm text-white/85">
